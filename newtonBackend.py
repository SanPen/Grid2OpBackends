...
try:
    ...
    NEWTONPA_ = False
    warnings.warn(
        "Newton cannot be loaded. You will gain possibly massive speed if installing it".format(sys.executable)
    )
...
class NewtonBackend(Backend):
    ...
    def load_grid(self, path=None, filename=None):
        ...
        """
        self._grid = newtonpa.FileHandler().open(file_name=filename)
        self.numerical_circuit = newtonpa.compileAt(circuit=self._grid, t=0)
